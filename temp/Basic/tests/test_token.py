...
def test_initial_state(token, owner):
    ...
    assert token.totalSupply() == 1000 * 10 ** 8
    assert token.balanceOf(owner) == 1000 * 10 ** 8
...
def test_transfer(token, owner, receiver):
    ...
    assert owner_balance == 1000 * 10 ** 8
    ...
    owner_balance = token.balanceOf(owner)
    assert owner_balance == 1000 * 10 ** 8 - 100
...
def test_transfer_from(token, owner, accounts):
    ...
    assert token.balanceOf(owner) == 1000 * 10 ** 8 - 300
